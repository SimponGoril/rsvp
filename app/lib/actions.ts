...
export async function insertAttendance(lessonsToAdd: Omit<LessonAttendence, 'id'>[]): Promise<LessonAttendence[] | undefined> {
    'use server'
    await supabase
        .from('attendance')
        .insert(lessonsToAdd)
...
export async function changeAttendance(id: number, email: string, will_attend: boolean): Promise<LessonAttendence[]> {
    ...
    if (isUnsigning) {
        return await handleUnsigning(lesson, lessons as LessonAttendence[]);
    } else {
        return await handleResigning(lesson, lessons as LessonAttendence[], id);
    }
};

async function handleResigning(attendance: LessonAttendence, lessons: LessonAttendence[], id: number) {
    'use server'
...
const handleUnsigning = async (attendance: LessonAttendence, lessons: LessonAttendence[]): Promise<LessonAttendence[]> => {
